...
func certInfoFromPEM(certParser *pki.CertParser, certPEM []byte) any {
	if len(certPEM) == 0 {
		return "<empty>"
	}

	cert, err := pki.ParseCertificateFromPEM(certParser, certPEM)
	if err != nil {
...
func certsInfoFromPEM(certParser *pki.CertParser, certsPEM []byte) any {
	if len(certsPEM) == 0 {
		return "<empty>"
	}

	certPool := pki.NewCertPool()
	if err := certPool.AddCertificatesFromPEM(certParser, certsPEM); err != nil {
